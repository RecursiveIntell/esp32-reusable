...
Fixed-capacity tiered-edge payloads.

![ri-esp-tiered architecture](assets/architecture.svg)

## What it provides

FeatureTransfer binary codec and wake/confidence helper.
...
```bash
cargo test -p ri-esp-tiered
cargo +esp check -p ri-esp-tiered --target xtensa-esp32s3-none-elf -Z build-std=core,alloc
```
